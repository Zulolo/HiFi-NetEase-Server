...
(arm64 today, riscv64 with the same binaries' sources), with MPD and the PWA usable on day 1
...
sequenceDiagram
    ...
    H->>M: addid http://127.0.0.1:8080/stream/ncm/123
...
| `hifid.service` | `hifid` (audio group, read access to `/proc/asound`) | 8080 (LAN), 8081/udp (discovery) | `After=mpd.service`, `Restart=always`, env file with the API token and encryption secret. |
| `avahi-daemon` | | 5353/udp | `_hifid._tcp`, `_http._tcp`, and host `.local` name. |
| `samba` (optional) | | 445 | share `/srv/music/local` for Explorer drag-and-drop. |

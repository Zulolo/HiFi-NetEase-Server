...
| R3 | Native DSD not offered by the kernel for a given dongle | Confirmed on the bench (Comtrue dongle on kernel 6.1 shows `SPECIAL`) and solved by the installer's `quirk_flags` handling; still open for unknown bridges | Falls back to DoP or PCM; DSD256 on CS43131 becomes software | `dac-setup.sh --try-native-dsd` (legacy and ≥ 6.18 forms); upstream patch; DoP works on all DSD dongles | done for the tested dongle |
...
| R16 | Out-of-tree Wi-Fi driver on the SBC (Zero 3: UWE5622; worse on XR819 boards) interrupts phone control, Samba copies or background downloads (local playback does not depend on Wi-Fi) | Medium | Annoyance, slow imports; never a playback dropout for local files | Watchdog timer restarts the link; power save off; offline-first sync with resumable downloads; PWA auto-reconnect; escalation: `mt76` USB adapter or Ethernet | M0 (soak pending) |
...
| Q11 | Exact small board? | First reported as a Zero LTS (512 MB); **the board on the bench identifies as Orange Pi Zero 3, Allwinner H618, 2 GB** (2026-09-24) | ADR-0007 revised: the Zero 3 is the verified first deployment; the 512 MB/XR819 concerns are withdrawn but kept as guidance for readers with such boards |
| Q12 | Wi-Fi bands / isolation? | **Separate 2.4 GHz and 5 GHz networks** | The Zero 3's UWE5622 supports both bands; phone, PC and board should share one LAN without client isolation so mDNS and Samba work |
| Q13 | `lsusb` IDs of the ES9039Q2M dongle | **Comtrue `2fc6:f802`, "XR768-9039"** (read from the bench) | Native DSD needs the installer's quirk on kernel 6.1 (mainline ≥ 6.14 has the Comtrue entry); verified DSD64/128 native, PCM to 768 kHz |
